...
> この Experiment は判断後に削除した記録である。
> variant のコードと preview は無い。本文が指す `variants/`、`shared/`、`previews/` は commit `eff731b` で辿る。
> 判断は [削除の ADR](../../decisions/2026-09-21-prune-decided-experiments.md) にある。

## Problem

公開 Catalog の見た目と情報設計を 1 から作り直す。
catalog-redesign（削除済み） の判断は、既存の殻 `hairline-float` を維持したまま `quiet-hierarchy` の役割と `playful-chroma` の色だけを借りる折衷だった。
...
| id            | 入口                                                         | 画面                               | 造形                         |
| ------------- | ------------------------------------------------------------ | ---------------------------------- | ---------------------------- |
| `topic-first` | 配色 / タイポグラフィ / トークン / コンポーネント / アイコン | トップ、トピック、詳細、見本、方針 | `issue-feature` から引き継ぐ |

却下した 4 案は評価に含めない。
実装は学習材料として残す（[ADR 2026-09-17](../../decisions/2026-09-17-named-variants-and-unevaluated-decisions.md)）。
...
| A5   | 配色トピックを coolors のパレットカードに寄せる   | 利用者が配色の面を確認する                           |
| A6   | 画面自体に正本の配色を当て、見本の文面を直す      | 利用者が最終案を決める                               |
| A7   | 方針ページの追加                                  | 利用者がガイドラインと画面を確認する                 |
| A8   | 方針ナビの親子化と方針画面の版面統一              | 利用者がナビと方針画面を確認する                     |
| A9   | サイドバーの整理と配色・明暗の選択のアイコン化    | 利用者がサイドバーと版面を確認する                   |
| A10  | 方針の書式をコアは思想、Tips は具体に分ける       | 利用者が新書式の文書と画面を確認する                 |
...
A8 では題字のナビを「成果物」「方針」の 2 親に畳み、方針の 6 文書をナビの子から直接開けるようにした。
...
2026-09-21 以降の方針画面とサイドバーの変更は公開面（`apps/catalog`）だけに入れており、`topic-first` の実装とは一致しない（[箱をやめる ADR](../../decisions/2026-09-21-catalog-guideline-plain-layout.md)）。

## Evaluation

評価は実施しない。
却下した 4 案は 2026-09-20 に評価を経ずに却下済みであり、比較する対象が残っていない（[named variants ADR](../../decisions/2026-09-17-named-variants-and-unevaluated-decisions.md)）。
...
判断は [topic-first ADR](../../decisions/2026-09-20-catalog-topic-first.md) に残す。
...
- 状態の表示に寸法を使うと、状態が変わるたびに画面が動く。ナビの現在地と variant の選択を太字にし、送りの数字を内容任せにしていたため、3 箇所でレイアウトシフトが出ていた。色と線で示し、数や語の場所は寸法を固定すれば起きない。原則候補として [docs/principles/state-changes-must-not-move-layout.md](../../principles/state-changes-must-not-move-layout.md) に抽出した。
- カタログが固有の色を持つと、成果物の配色を切り替えても変わらない面が残る。`--ed-*` を `--color-*` から導き、固有の紅を捨てたことで、10 配色それぞれで組んだ画面をそのまま見られるようになった。代償として、accent と text が同値の配色（`sumi`）では強調が文字と同じ色になる。
- 見本の文面は役割ごとに変える必要がある。すべての役割に「あAaＡ 日本語 UI」を当てていたため、文の長さと行数が役割によらず同じになり、行間や行長の値が画面から確かめられなかった。役割を持たない素の値（`font.size` など）だけは、字形を比べる並びのままでよい。
- 同じトピックの中で同じ内容を 2 度見せない。タイポグラフィは役割の見本を実寸で組んだ上に、同じ 6 役割を小さく見せる live 標本を並べていた。正本へ辿る道は 1 行のリンクで足りる。
- 見本を出しても、何で組んだのかは画面から読めない。役割の見本を 6 つ並べても書体名がどこにも無く、最も要る情報が落ちていた。名前は `tokens/typography/README.md` から読み、画面へ書き写さない。
- 値だけを並べると、どの数が何を指すかは読み手が推測することになる。「1.5rem 700 行 1.3 字間 0px」のうち、語が付いていた 行 と 字間 以外は読めなかった。値を増やさずに項目名を添えるだけで足りる。
...
- この案で作った値（送りのボタン 8rem、数字 4.5rem、角丸 0.375rem、最小ターゲット 2.5rem、transition 120ms）は token にしない。[Catalog ホストの ADR](../../decisions/2026-09-19-catalog-host.md) の規則 2 は「役割名があり、利用面が 2 つある」ことを求めるが、いずれも catalog-editorial の 1 面だけで使っている。角丸と影を token にしない判断（[soft-component-kit の ADR](../../decisions/2026-09-19-soft-component-kit.md)）とも同型で、案ごとに値が違う。段階 C で apps/catalog へ実装したときに、2 面になる値だけを改めて検討する。
...
- 方針を二次情報として載せると、成果物探索を邪魔せずに判断の根拠を確かめられる。
  正本は `docs/guidelines/` に置き、glob で読む。
  出どころを実験と外部出典に分け、規則に意図・根拠・良い例・悪い例を必須とした。
- 書式を厳格に定めた自前の軽量解析器により、外部依存なしで構造化して描画できる。
  不正な書式や未知の図版キーはビルド時に検出される。
- 図版は埋め込まず、キー参照（`- 図: <key>`）で描画する。
  文書と描画が疎結合になり、配色の追従やテーマ切替に対応できる。
- 層を名前だけで分けても、書き手は置き場に迷う。コアと Tips を同じ書式にしたところ、同じ規則が両方に書かれた。
...
- 前の判断: catalog-redesign（削除済み）
- 配色の正本: [color-schemes](../color-schemes/README.md)
- 文字の正本: [product-ui-typography](../../../experiments/product-ui-typography/README.md) と `tokens/typography/`
