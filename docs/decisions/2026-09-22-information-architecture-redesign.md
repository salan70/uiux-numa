...
- コアは 4 件にする。
  優先順は次のとおり。
  1. 直感的な名前で置き場所を 1 つに決める
  2. 判断に必要な情報から先に並べる
  3. 情報量に合わせて無駄な階層を挟まない
  4. 配置と移動の経路を固定する
- コアに数値を置かない。
- 既存 Tips はすべて捨てる。
...
2026-09-22 に利用者が確認し、コアの見出しと文面を推敲したうえで `status` を `adopted` にした。
...
- 旧 Tips「件数に見合った構造にする」は、絞り込みの Tip とコア「情報量に合わせて無駄な階層を挟まない」に置き換わった。
