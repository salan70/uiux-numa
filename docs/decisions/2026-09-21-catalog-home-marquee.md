...
- 帯のカードはリンクにしない。
- クリックしても遷移しない。
- 各成果物への入口は帯の下の WORKS に残す。
- 帯は `transform` ではなく横スクロールの位置を `requestAnimationFrame` で送る。
- ポインタが乗った段は徐々に減速して止まる。
- 帯の複製は `aria-hidden` にする。カードにリンクが無いので `tabindex` は付けない。
- `inert` は使わない。
- 複製に `inert` を付けると、その上でポインタが帯を突き抜け、止める操作が半分で届かなくなる。
...
同じフィードバックの続きで、帯のカードは押さないことにした。
流れる見本であり、遷移は WORKS から行う。
...
- `Link` の `tabIndex` は、帯の複製のリンクを Tab から外すためだけに足していた。
- カードからリンクを外したので、この引数も外した。
