...
- 例外: 文中のリンク、同じ機能を持つ十分な大きさの別の操作があるものなど、達成基準が定める例外。24×24 CSS px 未満の対象は、外接矩形の中心に置いた直径 24 CSS px の円が、他の対象にも他の小さな対象の円にも重ならない間隔を取った場合に限る。
- 実験: [skills/crafting-svg/references/ui-fit.md](../../skills/crafting-svg/references/ui-fit.md)
...
- 例外: 表、地図、図版など、2 次元の配置が内容の本質である要素は、Reflow（1.4.10）に限り 2 方向のスクロールを許す。表の中の文字も、200% の拡大で内容と機能を失わせない。
