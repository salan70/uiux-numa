...
意図と根拠: 色覚の多様性により、特定の色差を識別できない利用者がいる。
詳細はアクセシビリティのガイドラインを参照し、線種や語を併用する。

- 良い例: 状態の差異を色だけでなく下線やバッジの文字でも示す。
- 悪い例: ボタンの重要度を色の濃淡だけで表現する。
- 例外: なし。
- 実験: [アクセシビリティのガイドライン](accessibility.md)
...
- [ ] 色の区別に頼らず、テキストや記号を併用しているか。

## 出典

- [Design Tokens Community Group](https://design-tokens.github.io/community-group/format/): デザイントークンの役割と構造化
- [WCAG 2.2: 達成基準 1.4.1 / 1.4.3 / 1.4.11](https://www.w3.org/WAI/WCAG22/quickref/): 色の使用、最低限のコントラスト、非テキストのコントラスト
