...
個人開発のプロダクト群に共通する UI/UX の指針。現時点の記述は Web を前提とする。
書式と文書を足す手順は [guideline.md](../guideline.md) にある。

目的で自分のプロジェクトに関係するかを判断し、コアで思想と優先順位を読み、Tips で場面ごとの規則を引く。
Tips に無い場面はコアだけで判断する。
`draft` は人間の確認を経ていない仮説、`adopted` は確認済みである。
検証中の仮説は [原則候補](../principles/README.md) に置き、ガイドラインは詳細な値や検証結果を原則候補へ送る。
