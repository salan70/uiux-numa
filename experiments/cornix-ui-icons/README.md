...
- `keycap-tile`: 角丸の正方形の枠だけで、キーキャップらしさと 16px の読みやすさを両立できる。round 4 までの基準。
...
round 5 では、利用者が超楕円の形を選び、参考の写真（中央が浅く凹んだキーキャップ）を示した。
凹み（皿）は外形と同じ n の超楕円で、外形から d だけ内へ入れる。
...
| 5     | 利用者が超楕円の形を選び、中央が凹んだキーキャップの写真を示した                                                       | 角丸・台形・下の縁の 3 案を削除した。凹みを淡い面、淡い縁、上の縁の陰で描く 3 案を足した                 | ICON-06          | 利用者の確認待ち |

### 確認した内容

- round 5 の 68 個の原本（17 個 × 4 案）が `just svg-check --viewbox "0 0 24 24" --mono` を通った。
