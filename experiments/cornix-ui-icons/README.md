...
| rotate-cw   | 弧と頭                     | 弧             | 円の keyline      | r 6.75、中心 (12, 13.5) | 同じ                  | 0    | 頭を上に置くため、円を 1.5 下げて外形に収める                         |
| arrow-right | 右向きの矢印               | すべて         | `round-soft`      | `arrow-next` の値       | 同じ                  | 0    | 既存の組と同じ形にする                                                |
...
- `keycap-tile` は、Cornix の入口がすでに色のタイルの中にあるので、枠が二重になる。保存中の回転では枠ごと回る。
