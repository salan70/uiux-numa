...
- カバーが無いときは、同じ線と角丸と面の色の枠の中に文字を置く。hover ではカバーと同じく枠の中の文字を拡大する。
...
詳細の見本は Button と同じく、左に名前と説明と使う token、右に実物を置く行で並べる。token は各行 2 つまでに絞る。
文字だけのカードは、面の色をカバーと揃え、文字を左上起点で拡大する。中央起点だと左端の文字が枠の余白へ食い込む。
