...
| id                | 仮説                                                                   | 変えた軸                   | 実装                        |
| ----------------- | ---------------------------------------------------------------------- | -------------------------- | --------------------------- |
| `scale-card`      | HIG と同じくカード全体を 1.007 倍にすると、色を変えずに押せると示せる  | 動き: カード全体の拡大     | `variants/scale-card/`      |
| `zoom-cover`      | 枠を止めてカバーの中身だけを 1.04 倍にすると、並びを崩さず反応を示せる | 動き: カバーの中身だけ拡大 | `variants/zoom-cover/`      |
| `still-underline` | 何も動かさず題名の下線と枠線で返すと、一覧が最も落ち着く               | 動き: なし、下線と線の色   | `variants/still-underline/` |
