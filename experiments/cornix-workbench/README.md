...
| id           | 仮説                                                                                                                            | 変えた軸       | 実装                   |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------- | -------------- | ---------------------- |
| `board-desk` | 盤面を常に中央に置き、Overview・Behaviors・検証・実機を盤面の上に開く引き出しにすると、連続した割り当てを妨げずに他の作業へ届く | 情報構造、操作 | `variants/board-desk/` |

### `board-desk` の設計図

盤面、picker、編集パネルを常に同じ位置に置き、それ以外の作業は左端の入口から開く引き出しにする。
引き出しは盤面の上に重なり、盤面は背後に残る。
Apply だけは引き出しにせず、他の作業へ移れない modal にする。
...
│  └─ Drawer                      ← Overview / Behaviors / Validation / Device / Files（Drawers.tsx）
...
| W-06 全体を見渡す                       | 引き出し「全体」。参照元をたどると元の layer を開く               |
| W-07 Tap Dance、Combo、Settings         | 引き出し「動作」。範囲外の値は保存せず理由を出す                  |
| W-08、W-09 診断と参照                   | 引き出し「検証」、status bar の件数。診断から該当キーへ移る       |
| W-10〜12 接続、読込、Apply、backup 復元 | 引き出し「実機」の 3 段階、status bar の Apply、Apply の modal    |
| W-13、W-14、W-21 VIL、書出、再読込      | 引き出し「ファイル」、全体マップの書出                            |
| W-15 Karabiner                          | Mac のときの status bar と引き出し「実機」                        |
| W-16〜20 入口と復旧                     | WorkspaceGate、Recovery、編集パネルの保存状態                     |
| W-22 テーマ、ガイド、build              | header、引き出し「ファイル」                                      |
...
- 引き出しを開くと見出しへ focus し、Esc で閉じると入口へ戻ること。入力欄から focus が外れた後も Esc で閉じること。
