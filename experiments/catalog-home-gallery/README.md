...
- タイルは採用済みの `experiments/card/shared/Card.tsx` をそのまま使い、hover は `zoom-cover` を効かせる。Card の CSS を上書きしない。
- Card は 16:9 の枠と 3 行分の文字で高さが決まるので、大きさの差は並べる側が幅で付ける。同じ段のカードは同じ幅にして下端を揃える。
- 5 秒を超えて動き続ける内容には止める手段を置く。
- 帯の複製は `aria-hidden` にし、リンクを `tabindex="-1"` にして、読み上げと Tab を重複させない。`inert` にすると hover と押下も届かず、帯の半分が押せない面になるので使わない。
...
| id              | 仮説                                                                               | 変えた軸                                                 | 実装                      |
| --------------- | ---------------------------------------------------------------------------------- | -------------------------------------------------------- | ------------------------- |
| `marquee-rows`  | 逆向きに流れる帯を重ねると、スクロールしなくても成果物の幅が 1 画面で伝わる        | 並び: 段ごとに幅の違う 3 段の帯。動き: 無限の送り        | `variants/marquee-rows/`  |
| `mosaic-reveal` | 段抜きの幅で重要度を示す壁に、見えた順の登場を足すと、眺めながら順に読める         | 並び: 2 件、3 件、4 件の段を積む。動き: スクロールで登場 | `variants/mosaic-reveal/` |
| `depth-tilt`    | 層ごとの視差とタイルの傾きで奥行きを持たせると、静止した壁でも触れたくなる面になる | 並び: 同じ幅のグリッドに 3 つの層。動き: ポインタ追従    | `variants/depth-tilt/`    |

| id              | 向く状況                                                 | 代償                                                                                          |
| --------------- | -------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `marquee-rows`  | 件数が多く、1 画面で幅を見せたい。眺める時間が短い入口   | 常に動くので止める操作が要る。目当てのタイルが流れていき、狙って押しにくい                    |
| `mosaic-reveal` | 重要度に差をつけたい。スクロールして順に見てほしい       | 動きは登場の 1 回だけで、最初の画面の印象は静か。段の件数は並び順で決まり、内容で選んでいない |
...
- `depth-tilt`: 3 枚ごとに奥、中、手前の層を割り当て、奥の層は少し縮める。ポインタの位置で層ごとにずらし、乗ったカードの枠はその位置に向かって傾き、光の当たりが追う。
...
- `experiments/card/`: タイルは採用済みの `Card` と `zoom-cover` で描く。カバーの一部は `shared/covers.tsx` を使う。
