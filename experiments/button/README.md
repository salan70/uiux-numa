...
| `pill-zoom-label` | Card と同じく枠を止めて中身だけを拡大すると、部品間の hover の規則が揃う | hover: 文字とアイコンの拡大 | `variants/pill-zoom-label/`（hover の比較中） |
| `pill-scale`      | HIG と同じくボタン全体を拡大すると、影なしで押せると示せる               | hover: 全体の拡大           | `variants/pill-scale/`（hover の比較中）      |

`pill-zoom-label` と `pill-scale` は、採用した `pill-action` の hover だけを差し替えた比較案である。
形、色、押下の縮みは `pill-action` と同じにする。
判断後は却下した案のコードを削除する。
