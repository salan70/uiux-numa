...
採用後に hover の返し方を Card と揃えるか比べ、`pill-action` の hover だけを差し替えた 2 案を作った。
`pill-scale` を採用して `pill-action` へ取り込み、2 案のコードを削除した。

- `pill-zoom-label`: Card と同じく枠を止めて中身だけを拡大すると、部品間の hover の規則が揃う。変えた軸: hover で文字とアイコンを 1.05 倍にする。
- `pill-scale`: HIG と同じくボタン全体を拡大すると、影なしで押せると示せる。変えた軸: hover でボタン全体を 1.03 倍にする。
...
hover は `pill-scale` の案を採用し、1px リフトと影から全体の 1.03 倍の拡大に替える。

- 判断者: リポジトリの所有者
- 判断日: 2026-09-21
- 根拠: 利用者が現行、`pill-zoom-label`、`pill-scale` を runner で比べて選んだ。

## Rejected reasons

- `hairline-control`: 標準の角丸と境界線では、操作対象を明確にする形状の特徴が弱い。
- `tonal-layer`: 淡い面の差だけでは、役割の強さをボタン形状から読み取りにくい。
- hover の 1px リフトと影: Card の hover と揃えるため、`pill-scale` の全体の拡大に替えた。
- `pill-zoom-label`: 利用者は、文字だけを拡大する案より全体を拡大する `pill-scale` を選んだ。
...
頻繁に使う Button の hover は操作対象のフィードバックに限り、精密ポインタ上で `duration.press`(160ms)でボタン全体を 1.03 倍にする。
Card はカバーの中身、Button は全体と、拡大する範囲は部品ごとに違う。どちらも `duration.press` と `easing.out` を使う。
`prefers-reduced-motion` では拡大を止める。影は付けないため、reduced motion では hover の状態差が残らない。
